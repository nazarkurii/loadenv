...
func Test[T any](t *testing.T, loadFunc func(path ...string) (*T, error), envs Inputs) {
	...
	for _, env := range envs {
		if err := env.validate(); err != nil {
			t.Fatalf("provided test value error: %w", err)
		}
	}
	...
	testConfig[T](t, loadFunc, envs)
}

func testConfig[T any](t *testing.T, loadFunc func(path ...string) (*T, error), envs Inputs) {
	t.Run("testing from a env file", func(t *testing.T) {
		t.Parallel()
		testEnvs[T](t, loadFunc, envs, &FileEnvManager{})
	})

	t.Run("testing from system envs", func(t *testing.T) {
...
func testEnvs[T any](t *testing.T, loadFunc func(path ...string) (*T, error), envs Inputs, envManager Manager) {
	for testKey, testEnv := range envs {
		...
		if invalidValues := testEnv.invalidValues(); len(invalidValues) != 0 {
			t.Run("returns an error when "+testKey+" is invalid", func(t *testing.T) {
				envManager.New(t)

				for _, invalidGetValue := range invalidValues {
					for key, env := range envs {
						if key != testKey {
							envManager.SetEnv(key, env.getValues()[0])
						} else {
	...
	t.Run("succeeds when envs are set correctly", func(t *testing.T) {
		...
		for testedKey, testedEnv := range envs {
			values := testedEnv.getValues()

			length := len(values)
			for i := 0; i < length; i++ {
				defer envManager.Cleanup()
				envManager.SetEnv(testedKey, values[i])
				for key, env := range envs {
					if key != testedKey {
						envManager.SetEnv(key, env.getValues()[0])
					}
